...
public class GameServiceImpl implements GameService {
        ...
        return new Thread(() -> {
            ...
            Game newGame = Game.builder()
                               .gameDescription(GameDescription.builder()
                                                               ...
                                                               .currentPlayerId(clientListWithDescription.get(
                                                                       0)
    ...
    public synchronized void processMove(String gameId, ClientMessage clientMessage) {
        ...
        if (field[x][y].getBug() != null && (!field[x][y].getBug()
            ...
            if (!aliveAround.isEmpty()) {
                ...
                // сделать ход
                connectionService.broadcast(game.getPlayers(),
                                            makeMoveAndGetMessage(game,
                                                                  point,
                                                                  clientId,
                                                                  componentsAround,
                                                                  aliveAround));
                ...
                if (!allKilledByClientAround.isEmpty()) {
                    log.debug(
                            "Есть те, кого я убивал. Проверим, могу ли я рядом с ними встать (подключенность)");
    ...
    private ServerMessage makeMoveAndGetMessage(Game game, Point point, String clientId,
        ...
        if (field[x][y].isEmpty()) {
            ...
            List<FieldCell> componentsKilledBug = getAllKilledByClientBugsAround(field,
                                                                                 point,
                                                                                 field[x][y].getBug()
                                                                                            .getSetBy());
        ...
        if (game.getGameDescription()
            ...
            int nextPlayer = (game.getGameDescription()
                                  .getCurrentPlayerNumber() + 1) % game.getPlayers()
                                                                       .size();
        ...
        log.debug("Инкремент игрового счетчика: у игрока {} осталось ходов {}",
                  clientId,
                  game.getGameDescription()
                      .getRemainingMoves());
        ...
        for (int x = lowerBoundX; x <= upperBoundX; x++) {
            for (int y = lowerBoundY; y <= upperBoundY; y++) {
                ...
                if (!field[x][y].isEmpty() && field[x][y].getBug()
                                                         .isAlive() && clientId.equals(field[x][y].getBug()
                                                                                                  .getSetBy())) {
                    bugs.add(field[x][y]);
                }
            }
    ...
    private List<FieldCell> getAllKilledByClientBugsAround(FieldCell[][] field, Point point,
            String clientId) {
        List<FieldCell> bugs = new LinkedList<>();
